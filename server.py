    ...
    def reload_messages(self):
        ...
        if msgs_path.exists():
            ...
            if disk_mtime != getattr(self, '_last_mtime', None):
                ...
                # Check if owning claude -p process is still alive — if the _active_pid
                # file points to a dead process, the most recent _live marker is stale
                # (process died mid-stream) and must be cleaned or new generations wedge.
                ...
                if pid_path.exists():
                    try:
                        ...
                        pid_path.unlink(missing_ok=True)
                # Clean up stale _live markers — preserve most recent only if owner is alive
                _found_live = False
                ...
                # Recover from partial_response.json
                partial_path = self.dir / "partial_response.json"
                if partial_path.exists():
                    try:
                        partial = json.loads(partial_path.read_text())
        ...
        try:
          with self._jsonl_lock():
            ...
            print(f"[SESSION] Finalized live message at index {msg_index} "
                  f"({len(disk_lines)} total on disk, preserved)", flush=True)
        except Exception as e:
                        ...
                        if isinstance(block, dict):
                            if block.get("type") == "text":
                                ...
                                if block_thinking and len(block_thinking) > len(full_thinking):
                                    ...
                                    if _live_msg_idx >= 0 and _live_msg_idx < len(session.messages):
                                        session.messages[_live_msg_idx]["thinking"] = full_thinking
...
async def api_relay_clear_session(body: dict, auth_token: str = Cookie(None)):
    ...
    The assistant message that immediately follows each relay user-turn is also removed
    (when include_assistant_replies=True, default). Creates a timestamped .bak file first.
    Admin only. Per-session scoped.

    Body: { session_id: str, dry_run?: bool, include_assistant_replies?: bool }
    """
    user = get_user_from_token(auth_token)
    if not user:
        raise HTTPException(401, "Not authenticated")
...
async def api_get_session(session_id: str, limit: int = 50, offset: int = 0):
    ...
    # If generation is active, exclude the partial _live message — replay will rebuild it
    active_thread = getattr(session, '_active_thread', None)
    if active_thread and active_thread.is_alive():
        all_msgs = [m for m in all_msgs if not m.get("_live")]
    total = len(all_msgs)
    # offset=0 means "latest N". offset>0 means "older messages".
    # Return messages from the END (most recent), paginated backwards.
    if offset == 0:
...
#   RELAY_KEY_AI     = this instance's AI-side identity key
#   RELAY_KEY_HUMAN  = this instance's human-side identity key
